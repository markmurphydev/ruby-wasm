...
pub fn if_label_type_then_else() {
    ...
    let actual = &format!("{:?}", actual);
    let expected = expect![[r#"Instr { instr: If { label: Some("lab"), block_type: Some(Result(Num(I32))), then_block: [Instr { instr: Const { ty: I32, val: 1 }, folded_instrs: [] }], else_block: [Instr { instr: Const { ty: I32, val: 0 }, folded_instrs: [] }] }, folded_instrs: [] }"#]];
    expected.assert_eq(actual);
}
